...
const TeamForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (selectedTournament && teams.length >= selectedTournament.totalTeams) {
      alert("❌ Cannot add more teams. Team limit reached.");
      return;
    }
      ...
      {selectedTournamentId && (
        <>
          <div className="flex justify-between items-center mb-1">
            ...
            {selectedTournament.totalTeams - teams.length != 0 && (
              <button
              className={`px-4 py-2 rounded font-medium ${
                formVisible ? "bg-red-600" : "bg-blue-600"
              } text-white hover:opacity-90 transition`}
              onClick={() => {
                if (!formVisible && selectedTournament && teams.length >= selectedTournament.totalTeams) {
                  alert(`⚠️ Team limit reached! Max allowed: ${selectedTournament.totalTeams}`);
                  return;
                }
          ...
          {selectedTournament && (
            <p className="text-xl text-gray-600 mb-3">
              Remaining slots: {selectedTournament.totalTeams - teams.length}
            </p>
          )}
